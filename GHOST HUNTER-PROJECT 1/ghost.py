...
class Ghost:
    def __init__(self):
        random_size_value = random.uniform(GHOST_SIZE_RANDOMIZE[0], GHOST_SIZE_RANDOMIZE[1])
        size = (int(GHOSTS_SIZES[0] * random_size_value), int(GHOSTS_SIZES[1] * random_size_value))
        moving_direction, start_pos = self.define_spawn_pos(size)
        self.rect = pygame.Rect(start_pos[0], start_pos[1], size[0]//1.4, size[1]//1.4)
        self.images = [image.load("Assets/ghost/ghost.png", size=size, flip=moving_direction=="right")]
        self.current_frame = 0
        self.animation_timer = 0
    def define_spawn_pos(self, size): # define the start pos and moving vel of the ghost
        vel = random.uniform(GHOSTS_MOVE_SPEED["min"], GHOSTS_MOVE_SPEED["max"])
