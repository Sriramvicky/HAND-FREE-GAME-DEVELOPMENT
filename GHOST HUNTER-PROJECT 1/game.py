...
class Game:
    ...
    def spawn_insects(self):
        ...
        if t > self.insects_spawn_timer:
            self.insects_spawn_timer = t + GHOSTS_SPAWN_TIME
            nb = (GAME_DURATION-self.time_left)/GAME_DURATION * 100  / 1  # increase from 0 to 50 during all  the game (linear)
            if random.randint(0, 100) < nb:
                self.insects.append(Witch())
            else:
                self.insects.append(Ghost())
